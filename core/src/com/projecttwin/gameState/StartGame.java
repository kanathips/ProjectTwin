...
public class StartGame extends GameState implements EventListener {
	...
	public void create() {
		...
		for (int i = 0; i <= 16; i++)
			loading[i] = atlas.findRegion("load (" + (i + 1) + ")");
		...
		for (int i = 0; i <= 7; i++)
			lightFrame[i] = atlas_light.findRegion("light (" + (i + 1) + ")");
		...
		sprite_logo.setBounds(Constants.VIEWPORT_WIDTH / 12f, Constants.VIEWPORT_HEIGHT / 5f,
				sprite_logo.getWidth() + 250, sprite_logo.getHeight() / 2f + 150);

		TextureRegion[] startFrame = new TextureRegion[2];
		for (int i = 0; i <= 1; i++)
			startFrame[i] = atlas.findRegion("start" + i);
		...
		sprite_circle.get(0).setBounds(0 - sprite_circle.get(0).getWidth() / 2.3f,
				0 - sprite_circle.get(0).getHeight() / 2.3f, sprite_circle.get(0).getWidth() / 1.2f,
				sprite_circle.get(0).getHeight() / 1.2f);
		sprite_circle.get(0).setOriginCenter();

		sprite_circle.get(1).setBounds(Constants.VIEWPORT_WIDTH - sprite_circle.get(1).getWidth() / 2.9f,
				Constants.VIEWPORT_HEIGHT - sprite_circle.get(1).getHeight() / 2.9f,
	...
	public void render() {
		...
		// if(signal_load){
		// spriteBatch.draw(effect_ligh.getKeyFrame(deltaTime,true),
		// 0,Constants.VIEWPORT_HEIGHT/2.5f,1024,250);
		...
		// 0,0,Constants.VIEWPORT_WIDTH,Constants.VIEWPORT_HEIGHT);
		//
		// alpha_lo+= 0.005;
		...
		// if(alpha_lo == 1)
		// spriteBatch.draw(current_start,Constants.VIEWPORT_WIDTH/1.5f,0,
		// //Constants.VIEWPORT_HEIGHT/8f
